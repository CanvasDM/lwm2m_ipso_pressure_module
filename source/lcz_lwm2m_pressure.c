...
LOG_MODULE_REGISTER(lwm2m_pressure, CONFIG_LCZ_LWM2M_PRESSURE_LOG_LEVEL);
...
/**************************************************************************************************/
static struct lwm2m_obj_agent pressure_create_agent;

/**************************************************************************************************/
/* Local Function Prototypes                                                                      */
/**************************************************************************************************/
static int create_pressure_sensor(int idx, uint16_t type, uint16_t instance, void *context);

/**************************************************************************************************/
/* SYS INIT                                                                                       */
/**************************************************************************************************/
static int lcz_lwm2m_pressure_init(const struct device *dev)
{
	ARG_UNUSED(dev);

	pressure_create_agent.type = IPSO_OBJECT_PRESSURE_ID;
	pressure_create_agent.create = create_pressure_sensor;
	lcz_lwm2m_util_register_agent(&pressure_create_agent);

	return 0;
}

SYS_INIT(lcz_lwm2m_pressure_init, APPLICATION, LCZ_LWM2M_UTIL_USER_INIT_PRIORITY);
...
{
	uint16_t type = IPSO_OBJECT_PRESSURE_ID;
	int instance;
	int r;
	...
	r = lcz_lwm2m_pressure_set(instance, value);
...
int lcz_lwm2m_pressure_set(uint16_t instance, double value)
{
	...
	LCZ_SNPRINTK(path, "%u/%u/%u", IPSO_OBJECT_PRESSURE_ID, instance, SENSOR_VALUE_RID);
	return lwm2m_engine_set_float(path, &value);
}

int lcz_lwm2m_pressure_create(uint16_t instance)
{
	return lcz_lwm2m_util_create_obj_inst(IPSO_OBJECT_PRESSURE_ID, instance);
}
